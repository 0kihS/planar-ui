...
static void node_get_layout_info(struct wlr_scene_node *node, double *lx, double *ly, float *scale) {
	if (node->parent == NULL) {
		*lx = 0;
		*ly = 0;
		*scale = 1.0;
...
typedef bool (*scene_node_box_iterator_func_t)(struct wlr_scene_node *node,
	double sx, double sy, float scale, void *data);

static bool _scene_nodes_in_box(struct wlr_scene_node *node, struct wlr_box *box,
		scene_node_box_iterator_func_t iterator, void *user_data, double lx, double ly, float scale) {
	if (!node->enabled) {
		return false;
	}
	...
	case WLR_SCENE_NODE_TREE:;
		...
		wl_list_for_each_reverse(child, &scene_tree->children, link) {
			if (_scene_nodes_in_box(child, box, iterator, user_data, lx, ly, scale)) {
				return true;
			}
		}
		break;
	...
	case WLR_SCENE_NODE_BLUR:;
		...
		struct wlr_box node_box = {
			.x = floor(lx),
			.y = floor(ly),
			.width = ceil(lx + width * scale) - floor(lx),
			.height = ceil(ly + height * scale) - floor(ly),
		};

		if (wlr_box_intersection(&node_box, &node_box, box) &&
				iterator(node, lx, ly, scale, user_data)) {
			return true;
		}
		break;
	}
...
static bool scene_nodes_in_box(struct wlr_scene_node *node, struct wlr_box *box,
	...
	float ps = 1.0;
	if (node->parent) {
		node_get_layout_info(&node->parent->node, &plx, &ply, &ps);
	}

	return _scene_nodes_in_box(node, box, iterator, user_data, plx, ply, ps);
}

static void scene_node_opaque_region(struct wlr_scene_node *node, double x, double y,
		float scale, pixman_region32_t *opaque) {
	int width, height;
	scene_node_get_size(node, &width, &height);
	
	// Visual range: [x, x + width * scale]
	// Opaque integer range: [ceil(x), floor(x + width * scale)]
	int ox1 = ceil(x);
	int oy1 = ceil(y);
	int ox2 = floor(x + width * scale);
	int oy2 = floor(y + height * scale);
	...
	if (node->type == WLR_SCENE_NODE_RECT) {
		...
		if (!wlr_box_empty(&scene_rect->clipped_region.area)) {
			pixman_region32_fini(opaque);
			pixman_region32_init_rect(opaque, ox1, oy1, ox2 - ox1, oy2 - oy1);
			...
			pixman_region32_init_rect(&clipped_region, floor(clipped->x * scale) + x, floor(clipped->y * scale) + y,
					ceil(clipped->width * scale), ceil(clipped->height * scale));
		...
		if (!scene_buffer->buffer_is_opaque) {
			...
			wlr_region_scale(opaque, opaque, scale);
...
static bool scene_node_update_iterator(struct wlr_scene_node *node,
		double lx, double ly, float scale, void *_data) {
	...
	if (data->calculate_visibility && !data->optimized_blur_dirty) {
		...
		pixman_region32_init(&opaque);
		scene_node_opaque_region(node, lx, ly, scale, &opaque);
...
static void scene_node_bounds(struct wlr_scene_node *node,
		double x, double y, float scale, pixman_region32_t *visible) {
	if (!node->enabled) {
		return;
	}
	...
	if (node->type == WLR_SCENE_NODE_TREE) {
		...
		struct wlr_scene_node *child;
		wl_list_for_each(child, &scene_tree->children, link) {
			scene_node_bounds(child, x, y, scale, visible);
		}
		return;
	}
	...
	pixman_region32_union_rect(visible, visible, floor(x), floor(y),
		ceil(x + width * scale) - floor(x), ceil(y + height * scale) - floor(y));
}
...
static void scene_node_update(struct wlr_scene_node *node,
	...
	int ix, iy;
	if (!wlr_scene_node_coords(node, &ix, &iy)) {
#if WLR_HAS_XWAYLAND
	...
	// Calculate parent's total scale and layout coordinates
	double plx = 0, ply = 0;
	float ps = 1.0;
	if (node->parent) {
		node_get_layout_info(&node->parent->node, &plx, &ply, &ps);
	...
	scene_node_bounds(node, plx, ply, ps, &update_region);
...
void wlr_scene_buffer_set_buffer_with_options(struct wlr_scene_buffer *scene_buffer,
	...
	double lx, ly;
	float total_scale;
	node_get_layout_info(&scene_buffer->node, &lx, &ly, &total_scale);

	bool enabled = true;
	struct wlr_scene_node *it = &scene_buffer->node;
	while (it != NULL) {
	...
	if (scene_buffer->dst_width || scene_buffer->dst_height) {
		scale_x = (scene_buffer->dst_width * total_scale) / box.width;
		scale_y = (scene_buffer->dst_height * total_scale) / box.height;
	} else {
		scale_x = (buffer->width * total_scale) / box.width;
		scale_y = (buffer->height * total_scale) / box.height;
	}
	...
	wl_list_for_each(scene_output, &scene->outputs, link) {
		...
		pixman_region32_translate(&cull_region, floor(-lx * output_scale), floor(-ly * output_scale));
		pixman_region32_intersect(&output_damage, &output_damage, &cull_region);
		pixman_region32_fini(&cull_region);

		pixman_region32_translate(&output_damage,
			(int)floor((lx - scene_output->x) * output_scale),
			(int)floor((ly - scene_output->y) * output_scale));
		output_to_buffer_coords(&output_damage, scene_output->output);
		scene_output_damage(scene_output, &output_damage);
		pixman_region32_fini(&output_damage);
	}
...
void wlr_scene_buffer_set_opaque_region(struct wlr_scene_buffer *scene_buffer,
	...
	int ix, iy;
	if (!wlr_scene_node_coords(&scene_buffer->node, &ix, &iy)) {
		return;
	}
	...
	double parent_lx = 0, parent_ly = 0;
	float parent_scale = 1.0;
	if (scene_buffer->node.parent) {
		struct wlr_scene_node *curr = &scene_buffer->node.parent->node;
		parent_lx = curr->x;
		parent_ly = curr->y;
		float s = 1.0;
		while (curr->parent != NULL) {
			curr = &curr->parent->node;
			parent_lx = curr->x + parent_lx * curr->scale;
			parent_ly = curr->y + parent_ly * curr->scale;
			s *= curr->scale;
		}
		parent_lx -= scene_buffer->node.parent->node.x * s;
		parent_ly -= scene_buffer->node.parent->node.y * s;
		parent_scale = s;
	}

	scene_node_bounds(&scene_buffer->node, parent_lx, parent_ly, parent_scale, &update_region);
...
void wlr_scene_node_set_scale(struct wlr_scene_node *node, float scale) {
	if (node->scale == scale) {
		return;
	}
...
bool wlr_scene_node_coords(struct wlr_scene_node *node,
	...
	double lx, ly;
	float scale;
	node_get_layout_info(node, &lx, &ly, &scale);
...
static bool scene_node_at_iterator(struct wlr_scene_node *node,
		double lx, double ly, float scale, void *data) {
	struct node_at_data *at_data = data;

	double rx = (at_data->lx - lx) / scale;
	double ry = (at_data->ly - ly) / scale;
...
struct wlr_scene_node *wlr_scene_node_at(struct wlr_scene_node *node,
	...
	double nlx, nly;
	float ns;
	node_get_layout_info(node, &nlx, &nly, &ns);

	// Convert relative coordinates to layout coordinates
	// Note: nlx, nly are layout coordinates of 'node'.
	// ns is the total scale applied to 'node' by its ancestors.
	// But lx, ly are relative to 'node's origin.
	// So we scale them by 'ns' to get the layout-relative offset.
	double layout_x = nlx + lx * ns;
	double layout_y = nly + ly * ns;
...
static void scene_entry_render(struct render_list_entry *entry, const struct render_data *data) {
	...
	double lx = entry->x;
	double ly = entry->y;
	double x = lx - data->logical.x;
	double y = ly - data->logical.y;
	...
	struct wlr_box dst_box = {
		.x = floor(x),
		.y = floor(y),
		.width = ceil(x + width * scale) - floor(x),
		.height = ceil(y + height * scale) - floor(y),
	};
	transform_output_box(&dst_box, data);

	pixman_region32_t opaque;
	pixman_region32_init(&opaque);
	scene_node_opaque_region(node, floor(x), floor(y), scale, &opaque);
	...
	if (scene_output->scene->calculate_visibility) {
		for (int i = list_len - 1; i >= 0; i--) {
			...
			// rendering in that black rect region, consider the node's visibility.
					pixman_region32_t opaque;
					pixman_region32_init(&opaque);
					scene_node_opaque_region(entry->node, entry->x, entry->y, entry->scale, &opaque);			pixman_region32_intersect(&opaque, &opaque, &entry->node->visible);
